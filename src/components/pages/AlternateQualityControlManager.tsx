...
import {  altQcManagerState, roleBasedFilesState } from '../../recoil/state/formState';
...
const AlternateQualityControlManager= () => {
   const qc_name=useRecoilValue(altQcManagerState)
  const [heading, setHeading] = useState("5.Project Alternate Quality Control Manager\nINSERT RESUME & QUALIFICATIONS ["+qc_name+"]");
  const filesMap = useRecoilValue(roleBasedFilesState);
  const role = "alternateProjectManager";
...
export default AlternateQualityControlManager;
